...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_iter', type=int, default=350, help='number of iterations')
    ...
    parser.add_argument('--main_patience', type=int, default=C.main_patience)
    ...
    if dataset in {"kddcup2010"}:
        params.skill_emb_dim = 128
        params.answer_emb_dim = 64
        params.hidden_emb_dim = 128
        params.n_skill = 661
        params.batch_size = 24
        params.seqlen = 200
        params.data_dir = 'dataset/'+dataset
    ...
    for dataset_set_index in range(1,6):
        ...
        train_confidence=load_model.confidence_index(train_answer_data)
        train_difficulty=load_model.difficulty_index(train_skill_data,train_answer_data)
        val_confidence=load_model.confidence_index(val_answer_data)
        val_difficulty=load_model.difficulty_index(val_skill_data,val_answer_data)
        test_confidence=load_model.confidence_index(test_answer_data)
        test_difficulty=load_model.difficulty_index(test_skill_data,test_answer_data)
        
        early_stopping = EarlyStopping(patience=params.main_patience, verbose=True)
        ...
        for epoch in range(params.max_iter):
            ...
            train_confidence = train_confidence[shuffled_ind, :]
            train_difficulty = train_difficulty[shuffled_ind, :]
            ...
            for idx in range(train_N):
                ...
                train_batch_confidence = train_confidence[idx*params.batch_size:(idx+1)*params.batch_size]
                train_batch_difficulty = train_difficulty[idx*params.batch_size:(idx+1)*params.batch_size]
                
                skill = torch.LongTensor(train_batch_skill)
                answer = torch.LongTensor(train_batch_answer)
                confidence = torch.LongTensor(train_batch_confidence)
                difficulty = torch.LongTensor(train_batch_difficulty)

                skill = torch.where(skill==-1, torch.tensor([params.n_skill]), skill)
                answer = torch.where(answer==-1, torch.tensor([2]), answer)
                confidence = torch.where(confidence==-1, torch.tensor([1]), confidence)
                difficulty = torch.where(difficulty==-1, torch.tensor([1]), difficulty)
                for i in range(0,len(answer)):
                    for j in range(0,len(answer[0])):
                        if answer[i][j]==2 or answer[i][j]<0:
                            answer[i,j:]=2
                            skill[i,j:]=params.n_skill
                            break
                skill, answer = skill.to(device),answer.to(device)
                confidence,difficulty=confidence.to(device),difficulty.to(device)
                pred_res, features = net(skill, answer,confidence,difficulty)
                ...
                pred_res, features = net(skill, answer,confidence,difficulty, p_adv)
            ...
            with torch.no_grad():
                for idx in range(val_N):
                    ...
                    val_batch_confidence = val_confidence[idx*params.batch_size:(idx+1)*params.batch_size]
                    val_batch_difficulty = val_difficulty[idx*params.batch_size:(idx+1)*params.batch_size]
                    
                    skill=torch.LongTensor(val_batch_skill)
                    answer=torch.LongTensor(val_batch_answer)
                    confidence = torch.LongTensor(val_batch_confidence)
                    difficulty = torch.LongTensor(val_batch_difficulty)
                    skill = torch.where(skill==-1, torch.tensor([params.n_skill]), skill)
                    answer = torch.where(answer==-1, torch.tensor([2]), answer)
                    confidence = torch.where(confidence==-1, torch.tensor([1]), confidence)
                    difficulty = torch.where(difficulty==-1, torch.tensor([1]), difficulty)
        ...
        with torch.no_grad():
            for idx in range(test_N):
                ...
                test_batch_confidence = test_confidence[idx*params.batch_size:(idx+1)*params.batch_size]
                test_batch_difficulty = test_difficulty[idx*params.batch_size:(idx+1)*params.batch_size]
                
                skill=torch.LongTensor(test_batch_skill)
                answer=torch.LongTensor(test_batch_answer)
                confidence = torch.LongTensor(test_batch_confidence)
                difficulty = torch.LongTensor(test_batch_difficulty)
                skill = torch.where(skill==-1, torch.tensor([params.n_skill]), skill)
                answer = torch.where(answer==-1, torch.tensor([2]), answer)
                confidence = torch.where(confidence==-1, torch.tensor([1]), confidence)
                difficulty = torch.where(difficulty==-1, torch.tensor([1]), difficulty)
